...
function ServiceCard({ icon: Icon, number, title, description, span, image, index }) {
  return (
    <motion.div
      ...
      className={cn(
        'group relative bg-white border border-gray-100 rounded-xl p-7 lg:p-8 overflow-hidden',
        'hover:border-[#00A8E8]/30 hover:shadow-[0_8px_32px_rgba(0,168,232,0.08)]',
    ...
    >
      ...
      {/* Number watermark — hidden when image overlay is present */}
      {!image && (
        <span
          className="absolute top-5 right-6 text-6xl font-extrabold select-none pointer-events-none leading-none"
          style={{ color: 'rgba(0,168,232,0.07)' }}
        >
          {number}
        </span>
      )}
      ...
      {/* Icon */}
      <div className="w-11 h-11 rounded-lg bg-[#00A8E8]/8 flex items-center justify-center mb-6 group-hover:bg-[#00A8E8]/14 group-hover:scale-105 transition-all duration-300 relative z-10">
        <Icon size={20} className="text-[#00A8E8] group-hover:rotate-6 transition-transform duration-300" />
      </div>

      {/* Text */}
      <div className="relative z-10 max-w-xs">
        <h3 className="text-xl font-bold text-[#1a1a2e] mb-3">{title}</h3>
        <p className="text-[15px] text-gray-500 leading-7">{description}</p>
      </div>

      {/* Hover glow border */}
      <div
        className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
        style={{ boxShadow: 'inset 0 0 0 1.5px rgba(0,168,232,0.2)' }}
      />

      {/* Gold bottom accent — featured cards only */}
      {span === 2 && (
        <div
          className="absolute bottom-0 left-8 right-8 h-px opacity-0 group-hover:opacity-100 transition-opacity duration-400"
          style={{ background: 'linear-gradient(90deg, transparent, #F2D16B60, transparent)' }}
        />
      )}
    </motion.div>
  );
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-6 mb-12">
          <div className="max-w-xl">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[11px] font-bold text-[#00A8E8] uppercase tracking-widest">Our Services</span>
            </div>
          ...
          <div className="lg:max-w-sm">
            ...
            <Link href="/services" className="inline-flex items-center gap-1.5 mt-4 text-sm font-semibold text-[#00A8E8] hover:text-[#0096d4] transition-colors">
              View all services →
            </Link>
          </div>
        </div>
